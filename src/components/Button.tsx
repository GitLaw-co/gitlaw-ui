...
const {
  ...
  iconOnlySize: iconOnlyIconSizes,
  iconOnlyContainer: iconSizeClasses,
...
const variantClasses: Record<ButtonVariant, string> = {
  ...
  outline:
    "border border-primary text-foreground-button bg-transparent hover:bg-secondary/30",
...
export const Button: React.FC<ButtonProps> = ({
  ...
  const getIconColor = () => {
    ...
    if (["primary", "destructive"].includes(variant))
      return colors.textButtonNegative;
    return colors.textButton;
  };
  const iconColor = getIconColor();

  const baseClasses =
    "inline-flex items-center justify-center rounded font-normal transition-colors focus:outline-none focus:ring-2 focus:ring-primary/50";
  ...
  const renderLeftIcon = () => {
    ...
    if (leftIconName)
      return (
        <Icon
          name={leftIconName}
          className={isIcon ? iconOnlyIconSizes[size] : buttonIconSizes[size]}
          color={iconColor}
        />
      );
    return (
